...
public class MMCFC_DELUCAElliott_FRANCESKINNathan {
    ...
    public static boolean estCompat(int[] cod1, int[][] cod, int[][] rep, int nbCoups, int nbCouleurs) {
        ...
        */
        for (int i = 0; i < nbCoups; i++) {
            int commun = nbCommuns(cod1, cod[i], nbCouleurs);
            if (commun != (rep[i][0]+rep[i][1])) { // L'on ne cherche plus à les mettre en ordre donc les couleurs communes suffisent.
                return false;
            }
        }
        return true;
    ...
    */
    ...
    /**
    LOGIQUE CFC : le fond est le meme il est 
    */
    public static int mancheOrdinateur(int lgCode, char[] tabCouleurs, int numManche, int nbEssaisMax) {
        ...
        boolean FC = false; // si false alors phase C ; si true phase FC
        int[] Ccode = new int[lgCode]; // code à la fin de la phase C.
        int[] curseur = {-1,-1,-1}; // curseur[0] = couleur ; curseur[1] = position curseur dans propCode ; curseur[2] = position curseur dans Ccode
        int fond = -1; // couleur du fond
        int[] codeFinal = initTab(lgCode,-1); // le dernier code qui sera proposé et il sera juste.
        ...
        do {
            ...
            //rep[nbEssais] = nbBienMalPlaces(codeSolution,cod[nbEssais],tabCouleurs.length); // pour repondre automatiquement.
            ...
            if (!victoire && FC){ //ce code ne s'execute que lors de la phase FC
            boolean passeCurseur = genereCodFinal(codeFinal, curseur, fond, rep[nbEssais-1]);

                if (!victoire && sontEgaux(propCode, codeFinal)){ // le code final doit etre juste sinon incohérence
                    incoherence = true;
                }
                ...
                if (passeCurseur) { 
                    do  {
                        curseur[2]++;
                        if (curseur[2] == lgCode){  // le cuseur dépasse
                            curseur[2] = lgCode-1; // si le cuseur dépasse alors le code doit etre finalisé j'applique donc la condition de finaliseCodFinal() et je sors
                            break; // je sais, c'est mal ! mais là j'en avais besoin pour aller plus vite.
                        }
                        curseur[1] = 0; // le curseur change donc la positon doit etre remise à 0
                        curseur[0] = Ccode[curseur[2]]; // le curseur change donc il prend la nouvelle couleur.
                    } while (curseur[0] == fond);
                    
                } else {
                    curseur[1]++; //si on ne change pas de couleur de curseur alors on change sa place dans cod
                }

                if (curseur[2] == lgCode-1) { // ceci est la condition que je cite plus haut
                    propCode = finaliseCodFinal(codeFinal,fond);
                } else {
                    passeCodeSuivantFC(curseur,fond,propCode);
                }
            }
            if (!victoire && !FC) {
                FC = rep[nbEssais-1][0]+rep[nbEssais-1][1] == lgCode; //debut de la phase FC si vrai
                if (!FC) { // ce code ne s'execute que dans la phace C.
                ...
                } else { // le code qui suit ne s'execute que lors du premier passage dans la phase FC.
                    Ccode = copieTab(cod[nbEssais-1]);
                    curseur[1] = 0; //set position curseur dans propCode
                    curseur[2] = 0; //set position curseur dans Ccode
                    curseur[0] = Ccode[curseur[2]]; //set couleur curseur
                    fond = Ccode[lgCode-1]; //set couleur fond (ne change pas)
                    passeCodeSuivantFC(curseur,fond,propCode);
                }
                clearConsole();
            }
        ...
        for (int i = 0; i <= nbCoups; i++) {
            ...
            if (!sontEgaux(repValide, rep[i])) {
                ...
                if (rep[i][0] != repValide[0]) {
                    System.out.println("    Le nombres de couleurs bien placé est faux");
                }
                if (rep[i][1] != repValide[1]) {
                    System.out.println("    Le nombres de couleurs mal placé est faux");
                }
    ...
    */
    public static void statsMasterMindIA(){
        ...
        do{
            ...
            boolean FC = false;
            int[] Ccode = new int[lgCode];
            int[] curseur = {-1,-1,-1};
            int fond = -1;
            int[] codeFinal = initTab(lgCode,-1);

            int nbEssais = 0;
            do {
                cod[nbEssais] = copieTab(propCode);
                rep[nbEssais] = nbBienMalPlaces(codeSolution,cod[nbEssais],nbCouleurs); // pour repondre automatiquement.
                victoire = rep[nbEssais][0] == lgCode;
                nbEssais++;
                if (!victoire && FC){
                boolean passeCurseur = genereCodFinal(codeFinal, curseur, fond, rep[nbEssais-1]);
                    ...
                    if (passeCurseur) { 
                        do  {
                            ...
                            curseur[1] = 0;
                            curseur[0] = Ccode[curseur[2]];
                        } while (curseur[0] == fond);

                    } else {curseur[1]++;}
                    
                    if (curseur[2] == lgCode-1) {
                        propCode = finaliseCodFinal(codeFinal,fond);
                    } else {
                        passeCodeSuivantFC(curseur,fond,propCode);
                    }
                ...
                if (!victoire && !FC) {
                    FC = rep[nbEssais-1][0]+rep[nbEssais-1][1] == lgCode;
                    if (!FC) {
                        incoherence = !passeCodeSuivantLexicoCompat(propCode, cod, rep, nbEssais, nbCouleurs);
                    } else {
                        Ccode = copieTab(cod[nbEssais-1]);
                        curseur[1] = 0; curseur[2] = 0; curseur[0] = Ccode[curseur[2]];
                        fond = Ccode[lgCode-1];
                        passeCodeSuivantFC(curseur,fond,propCode);
                    }
                }
        ...
        } while (!sontEgaux(codeSolution, DernierCodeSolution));
    ...
    /**
    genereCodFinal ce lance apres chaque poposition de code.
    ACTION : détermine grace a la position des couleurs a partir du curseur et du fond et modifie codFinal avec ses déductions.
    RESULAT : un booleen, true si le cuseur doit changer sinon false dans le cas ou il ne déduit rien.
    POURQUOI JE RETURN TRUE : des qu'il trouve il passe au cuseur suivant soit dans cet exemple de Ccode : RRBV, le premier cuseur sera R puis R puis B et ne sera jamais V.
    */

    public static boolean genereCodFinal(int[] codFinal,int[] curseur, int fond ,int[] rep){
        if (rep[1] == 0 && codFinal[curseur[1]] == -1) {
            //alors curseur bien placé
            codFinal[curseur[1]] = curseur[0]; // j'applique la couleur du curseur dans la case correspondante dans codFinal
            return true; 
        } else if (curseur[1] == codFinal.length-2){ // si le curseur n'a pas trouvé a l'avant derniere valeur alors elle est sur la derniere
            codFinal[codFinal.length-1] = curseur[0];
            return true;
        }
        return false;
    }
    ...
    /**
    finaliseCodFinal ce lance quand toute les autres couleurs ont été trouvées sauf fond
    ACTION : met la couleur fond dans toutes les cases manquante (-1).
    RESULTAT : il retourne le codFinal qui est obligatoirement juste.
    */

    public static int[] finaliseCodFinal(int[] codFinal, int fond){
        for (int i = 0; i < codFinal.length; i++) {
            if (codFinal[i] == -1) {
                codFinal[i] = fond;
            }
        }
    ...
    /**
    ACTION : met la couleur fond dans toutes les cases sauf celle avec le curseur.
    RESULTAT : modifie porpCode sans le retourner.
    */
    
    public static void passeCodeSuivantFC(int[] curseur, int fond, int[] porpCode) {
        for (int i = 0; i < porpCode.length; i++) {
            if (curseur[1] == i) {
                porpCode[i] = curseur[0];
            } else {
                porpCode[i] = fond;
            }
        }
