...
public class MMCSJO_DELUCAElliott_FRANCESKINNathan {
    ...
    public static int[] tabFrequenceInverse(int[] cod, int nbCouleurs) {
        int[] repet = new int[nbCouleurs];
        for (int i = 0; i < cod.length; i++) {
            repet[cod[i]]--; // cod[i] = indice de la couleur dans tabCouleurs et l'indice de repet = indice de la couleur. +1 par répétition.
        }
    ...
    public static boolean passeCodeSuivantLexicoInverse(int[] cod1, int nbCouleurs) {

        for (int i = cod1.length - 1; i >= 0; i--) {
            cod1[i] = Math.floorMod((cod1[i] - 1), nbCouleurs);  // augmente de 1 la valeur et 0 si egal a la base
            if (cod1[i] != nbCouleurs-1) { // il n'y a pas de retenu donc fin
                return true;
            }
        }
        return false; // Si que des 0 alors non réalisable.
    ...
    */
    public static boolean passeCodeSuivantLexicoCompat(int[] cod1, int[][] cod, int[][] rep, int nbCoups, int nbCouleurs) {
        ...
        do { // il doit le faire au moins une fois car le premier est forcement incompatible.
            ...
            if (!sontEgaux(cod1, cod[nbCoups-1])) { // si il ne revient pas au code de départ alors le code existe.
                codeExiste =true;
            }
    ...
    public static boolean passeCodeSuivantLexicoCompatInverse(int[] cod1, int[][] cod, int[][] rep, int nbCoups, int nbCouleurs) {
        boolean codeExiste = false;
        do { // il doit le faire au moins une fois car le premier est forcement incompatible.
            passeCodeSuivantLexicoInverse(cod1, nbCouleurs);
            if (!sontEgaux(cod1, cod[nbCoups-1])) {  // si il ne revient pas au code de départ alors le code existe.
                codeExiste =true;
            }
        } while (!estCompat(cod1, cod, rep, nbCoups, nbCouleurs) && codeExiste);
    ...
    public static int mancheOrdinateur(int lgCode, char[] tabCouleurs, int numManche, int nbEssaisMax) {
        ...
        boolean aleatoire = randomMinMax(0, 1) == 1;

        //si 1 couleur alors pas besoin d'aléatoire
        if(tabCouleurs.length==1){
            aleatoire = false;
        }

        if(aleatoire){
            propCode = copieTab(codeAleat(lgCode, tabCouleurs.length-1));
        }
        ...
        do {
            ...
            if (!victoire) {
                if(randomMinMax(0,1)==0 || aleatoire){
                    ...
                    passeCodeSuivantLexicoCompatInverse(propCode, cod, rep, nbEssais, tabCouleurs.length); // si code n'existe pas alors incohérence
                }
                clearConsole();
            }
    ...
    */
    public static void statsMasterMindIA(){
        ...
        if (nbCouleurs > 1){
            System.out.println("aleatoire (1 = oui ; 2 = non) :");
            
            do {
                saisie = saisirEntierPositif();
            } while (saisie > 2);
            if (saisie == 1) {
                aleatoire = true;
            }
        ...
        if (!aleatoire){
            System.out.println("alterne lexico inverse et pas inverse (1 = oui ; 2 = non) :");
        ...
        int[] DernierCodeSolution = new int[lgCode]; // un tableau de 0 qui ne change pas il simplifie juste la condition d'arret du while.
        ...
        do { // teste chaque code et prend le max
            ...
            int[] propCode = new int[lgCode];
            if (aleatoire) {
                propCode = copieTab(codeAleat(lgCode, nbCouleurs));
            }
            int[][] cod = new int[nbEssaisMax][lgCode];
            int[][] rep = new int[nbEssaisMax][2];
            boolean victoire;
            ...
            do { // joue marche ordinateur (allégé)
                System.out.println("code en cours :"+listElem(codeSolution));
                cod[nbEssais] = copieTab(propCode);
                rep[nbEssais] = nbBienMalPlaces(codeSolution,cod[nbEssais],nbCouleurs);
                victoire = rep[nbEssais][0] == lgCode;
                nbEssais++;
                if (!victoire) {
                boolean tourInverse = false;
                if (inverse) {
                    tourInverse = randomMinMax(0,1) == 0;
                }
                if(!tourInverse || aleatoire){
                    System.out.println("Passe code");
                    passeCodeSuivantLexicoCompat(propCode, cod, rep, nbEssais, nbCouleurs); // si code n'existe pas alors incohérence
                }else{
                    System.out.println("Passe code Inverse");
                    passeCodeSuivantLexicoCompatInverse(propCode, cod, rep, nbEssais, nbCouleurs); // si code n'existe pas alors incohérence
                }
                clearConsole();
            }
